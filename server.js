...
io.on('connection', (socket) => { 
    ...
    // 1. Usuário abre o chamado e entra na sala exclusiva dele
    socket.on('entrar_chamado', (chamado_id) => {
        socket.join(`chamado_${chamado_id}`);
        console.log(`Usuário entrou na sala do chamado ${chamado_id}`);
    });

    // 2. Usuário fecha o modal do chamado e sai da sala
    socket.on('sair_chamado', (chamado_id) => {
        socket.leave(`chamado_${chamado_id}`);
        console.log(`Usuário saiu da sala do chamado ${chamado_id}`);
    });

    // 3. Usuário envia uma mensagem no chat
    socket.on('enviar_mensagem', async (dados) => {
        const { chamado_id, remetente_nome, remetente_email, mensagem } = dados;

        try {
            // --- NOVA REGRA: VERIFICA O STATUS DO CHAMADO ANTES DE ENVIAR ---
            ...
            // NOVA REGRA FLEXÍVEL NO BACKEND
            ...
            // A) Emite a mensagem IMEDIATAMENTE para a sala
...
// Carregar histórico de chat de um chamado específico
...
// CRIAÇÃO DE CHAMADO COM NOTIFICAÇÃO
app.post('/api/chamados', async (req, res) => { 
    ...
    try { 
        ...
        // --- EVENTOS DISPARADOS PARA ATUALIZAR AS TELAS ---
        io.emit('atualizar_kanban'); 
        io.emit('novo_chamado'); // <--- AVISA A TELA DE MEUS CHAMADOS
        // --------------------------------------------------
...
app.put('/api/chamados/:id', async (req, res) => { 
    ...
    try { 
        ...
        if (novosDados.status === 'concluido' && atual.status !== 'concluido') { 
            novosDados.data_conclusao = new Date(); 
        ...
        // --- EVENTOS DISPARADOS PARA ATUALIZAR AS TELAS ---
        io.emit('atualizar_kanban'); 
        io.emit('chamado_atualizado'); // <--- AVISA A TELA DE MEUS CHAMADOS
        // --------------------------------------------------
...
app.delete('/api/chamados/:id', async (req, res) => { 
    try { 
        ...
        // --- EVENTOS DISPARADOS PARA ATUALIZAR AS TELAS ---
        io.emit('atualizar_kanban'); 
        io.emit('chamado_excluido'); // <--- AVISA A TELA DE MEUS CHAMADOS
        // --------------------------------------------------
